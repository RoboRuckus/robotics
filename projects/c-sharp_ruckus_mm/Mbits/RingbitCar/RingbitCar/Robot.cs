...
namespace RingbitCar
{
    ...
    public class Robot
    {
        ...
        public Robot()
        {
            ...
            // Attach servos
            _left = new ServoMotor(PwmChannel.CreateFromPin(_leftPin, 50), 180, 500, 2500);
            _right = new ServoMotor(PwmChannel.CreateFromPin(_rightPin, 50), 180, 500, 2500);
            // Make sure motors are stopped
            _left.WriteAngle(90);
            _right.WriteAngle(90);
        }
        ...
        public void DriveForward(int magnitude, int lateralMove)
        {
            ...
            _gyro.ResetGyroDegrees();
            while (DateTime.UtcNow.Ticks - start < total)
            {
                gyroX = _gyro.getGyroDegrees()[0];
                /*
                ...
                if (gyroX > _drift)
                {
                    rightSpeed = _rightForwardSpeed - _driftBoost;
                    leftSpeed = _leftForwardSpeed;
                }
                else if (gyroX < -_drift)
                {
                    rightSpeed = _rightForwardSpeed;
                    leftSpeed = _leftForwardSpeed + _driftBoost;
                }
                else
                {
                    rightSpeed = _rightForwardSpeed;
                ...
                _left.WriteAngle(leftSpeed);
                _right.WriteAngle(rightSpeed);
                Thread.Sleep(50);
            }
            // Stop motors
            _left.WriteAngle(90);
        ...
        public void DriveBackward(int magnitude, int lateralMove)
        {
            ...
            while (DateTime.UtcNow.Ticks - start < total)
            {
                ...
                if (gyroX > _drift)
                {
                    rightSpeed = _rightBackwardSpeed;
                    leftSpeed = _leftBackwardSpeed - _driftBoost;
                }
                else if (gyroX < -_drift)
                {
                    rightSpeed = _rightBackwardSpeed + _driftBoost;
                    leftSpeed = _leftBackwardSpeed;
                }
                else
                {
                    rightSpeed = _rightBackwardSpeed;
        ...
        public void Turn(int magnitude, int direction, int lateralmove)
        {
            ...
            if (direction == 0)
            {
                _left.WriteAngle(_leftForwardSpeed);
                _right.WriteAngle(_rightBackwardSpeed);
            }
            else if (direction == 1)
            {
                _left.WriteAngle(_leftBackwardSpeed);
                _right.WriteAngle(_rightForwardSpeed);
            }
            else
            {
                // Bad command, exit
            ...
            while (Math.Abs(_gyro.getGyroDegrees()[0]) < target)
            {
                Thread.Sleep(20);
            }
            // Stop motors
